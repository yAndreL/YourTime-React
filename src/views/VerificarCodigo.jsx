          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <div className="flex justify-center gap-2 mb-4" onPaste={handlePaste}>
                {codigo.map((digit, index) => <input key={index} ref={inputRefs[index]} type="text" maxLength={1} value={digit} onChange={e => handleChange(index, e.target.value)} onKeyDown={e => handleKeyDown(index, e)} disabled={loading} className={`w-12 h-14 text-center text-2xl font-bold border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all disabled:bg-gray-100 disabled:cursor-not-allowed ${erro ? 'border-red-500' : 'border-gray-300'}`} autoFocus={index === 0} />)}
              </div>
              
              {erro && <p className="text-sm text-red-600 text-center flex items-center justify-center gap-1">
                  <FiAlertCircle className="w-4 h-4" /> {erro}
                </p>}

              {mensagemSucesso && <p className="text-sm text-green-600 text-center flex items-center justify-center gap-1">
