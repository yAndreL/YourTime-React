...
import { limparRecuperacao, lerTempToken } from '../utils/passwordRecoveryStorage';
import { postAuthReset } from '../services/ApiService';
...
function ResetarSenha() {
  ...
  const [useRecoveryApi, setUseRecoveryApi] = useState(false);
  ...
  useEffect(() => {
    ...
    const tentarAtivarFluxoCodigoVerificado = () => {
      const estadoNavegacao = location.state;
      const dadosArmazenados = lerTempToken();
      const tokenResolvido = estadoNavegacao?.temp_token || dadosArmazenados?.temp_token;
      ...
      if (tokenResolvido) {
        setEtapaRedefinicaoSenha('codigo_verificado');
        setUseRecoveryApi(true);
        return true;
      }
      if (emailResolvido && estadoNavegacao?.codigoVerificado) {
        setEtapaRedefinicaoSenha('codigo_verificado');
        setUseRecoveryApi(true);
        return true;
      }
    ...
    if (tentarAtivarFluxoCodigoVerificado()) {
      return () => {
  ...
  const processarDefinicaoNovaSenha = async eventoEnvio => {
    ...
    try {
      ...
      if (etapaRedefinicaoSenha === 'codigo_verificado' && useRecoveryApi) {
        const estadoNavegacao = location.state;
        const dadosArmazenados = lerTempToken();
        const temp_token = estadoNavegacao?.temp_token || dadosArmazenados?.temp_token;

        if (!temp_token) {
          setMensagemErroFormulario('Sessao expirada. Solicite um novo codigo.');
          setCarregandoAtualizacaoSenha(false);
          return;
        }

        const resultado = await postAuthReset(temp_token, novaSenhaDigitada);
        if (resultado.success) {
          limparRecuperacao();
          setRedefinicaoConcluidaComSucesso(true);
          setTimeout(() => navigate('/login', { replace: true }), 2500);
        } else {
          setMensagemErroFormulario(resultado.error || t('validacao.errorUpdatingPassword'));
          setCarregandoAtualizacaoSenha(false);
          return;
        }
        return;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-100 via-slate-200 to-slate-300 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950 flex items-center justify-center p-4">
        <div className="w-full max-w-md text-center yt-modal-surface rounded-xl shadow-2xl p-8">
          <p className="text-gray-700 dark:text-gray-300 mb-4">Sessao expirada ou invalida. Redirecionando...</p>
          ...
          <div className="yt-modal-surface rounded-xl shadow-2xl p-8">
            <div className="text-center mb-6">
              <div className="w-20 h-20 bg-green-100 dark:bg-green-950/50 rounded-full flex items-center justify-center mx-auto mb-4">
                <FiCheckCircle className="w-12 h-12 text-green-600 dark:green-400" />
              </div>
            ...
            <div className="bg-green-50 dark:green-950/30 border border-green-200 dark:border-green-800 rounded-lg p-4 mb-6">
