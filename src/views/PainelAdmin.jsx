...
function PainelAdministrativo() {
  ...
  const carregarDiasComPontosPendentes = async () => {
    try {
      ...
      const {
        data: agendamentos,
        error
      } = await supabase.from('agendamento').select('data, status, user_id, superior_empresa_id').eq('status', 'P').eq('superior_empresa_id', superiorEmpresaId).order('data', {
        ascending: false
      });
      if (error) throw error;
  ...
  const carregarFuncionarios = async () => {
    try {
      ...
      const {
        data: todosPontosDia
      } = await supabase.from('agendamento').select('*').eq('data', dataSelecionada).in('user_id', funcionarioIds);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="border-b border-gray-200">
          <nav className="flex -mb-px">
            <button onClick={() => setAbaAtiva('funcionarios')} className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-6 py-2 sm:py-4 border-b-2 font-medium text-xs sm:text-sm transition-colors ${abaAtiva === 'funcionarios' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
              <FiUsers className="w-4 h-4 sm:w-5 sm:h-5" />
              {t('admin.employees')}
            </button>
            <button onClick={() => setAbaAtiva('removidos')} className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-6 py-2 sm:py-4 border-b-2 font-medium text-xs sm:text-sm transition-colors ${abaAtiva === 'removidos' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
              <FiLock className="w-4 h-4 sm:w-5 sm:h-5" />
              {t('admin.removed')}
            </button>
            <button onClick={() => setAbaAtiva('empresas')} className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-6 py-2 sm:py-4 border-b-2 font-medium text-xs sm:text-sm transition-colors ${abaAtiva === 'empresas' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
        ...
        <div className="p-3 sm:p-6">
          {abaAtiva === 'funcionarios' ? <div>
              ...
              {diasComPontosPendentes.length > 0 && <div className="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded-lg">
                  <div className="flex items-start">
                    ...
                    <div className="ml-3 flex-1">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {diasComPontosPendentes.slice(0, 5).map((data, index) => <button key={data} onClick={() => selecionarData(data)} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${dataSelecionada === data ? 'bg-yellow-600 text-white' : 'bg-white text-yellow-700 hover:bg-yellow-100 border border-yellow-300'}`} title={`${t('admin.filterByDate')} ${formatarData(data)}`}>
                            {formatarData(data)}
                          </button>)}
                        {diasComPontosPendentes.length > 5 && <>
                            <span className="px-3 py-1.5 text-sm text-yellow-700">...</span>
                            <div className="relative group">
                              <button className="px-3 py-1.5 bg-yellow-600 text-white rounded-md text-sm font-medium hover:bg-yellow-700 transition-colors">
                              ...
                              <div className="absolute left-0 top-full mt-2 w-64 bg-white border border-yellow-300 rounded-lg shadow-lg p-3 z-10 hidden group-hover:block">
                                ...
                                <div className="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto">
                                  {diasComPontosPendentes.slice(5).map(data => <button key={data} onClick={() => selecionarData(data)} className="px-2 py-1 bg-yellow-50 text-yellow-700 hover:bg-yellow-100 border border-yellow-200 rounded text-xs" title={`${t('admin.filterByDate')} ${formatarData(data)}`}>
                                      {formatarData(data)}
                                    </button>)}
                                </div>
                              </div>
                ...
                <div className="flex flex-col lg:flex-row gap-2 sm:gap-3 compact:gap-1 items-stretch lg:items-center justify-between">
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 compact:gap-1 items-stretch sm:items-center">
                    <div className="flex items-center gap-1 sm:gap-2 compact:gap-1">
                      ...
                      <input type="text" value={buscaTexto} onChange={e => setBuscaTexto(e.target.value)} placeholder={t('admin.searchPlaceholder')} className="w-full sm:w-40 md:w-48 lg:w-56 xl:w-64 px-1.5 sm:px-2 lg:px-3 py-1 sm:py-1.5 lg:py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-[11px] sm:text-xs lg:text-sm" />
                    </div>
                    ...
                    <div className="flex items-center gap-1 sm:gap-2 compact:gap-1">
                      ...
                      <input type="date" value={dataSelecionada} onChange={e => setDataSelecionada(e.target.value)} className="w-full sm:w-auto px-1.5 sm:px-2 lg:px-3 py-1 sm:py-1.5 lg:py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-[11px] sm:text-xs lg:text-sm" />
                    </div>
                  </div>

                  <button onClick={() => navigate('/cadastro')} className="w-full sm:w-auto px-2 sm:px-3 lg:px-4 compact:px-3 py-1 sm:py-1.5 lg:py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors flex items-center justify-center gap-1 sm:gap-2 compact:gap-1 text-[10px] sm:text-xs lg:text-sm whitespace-nowrap">
              ...
              <div className="overflow-x-auto mt-6 -mx-3 sm:-mx-4 lg:mx-0">
                {loading ? <div className="text-center py-8">
                  ...
                  </div> : <table className="w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {funcionariosFiltrados.length === 0 ? <tr>
                        ...
                        </tr> : funcionariosFiltrados.map(funcionario => <tr key={funcionario.id} className="hover:bg-gray-50">
                            <td className="px-0 sm:px-1 lg:px-2 py-2 sm:py-3 lg:py-4 whitespace-nowrap relative">
                              <button onClick={() => setMenuAberto(menuAberto === funcionario.id ? null : funcionario.id)} className="p-0.5 sm:p-1 hover:bg-gray-100 rounded transition-colors" title="Opções">
                            ...
                            <td className="px-0 sm:px-2 md:px-3 lg:px-4 xl:px-6 py-2 sm:py-3 lg:py-4 whitespace-nowrap text-[9px] sm:text-[10px] md:text-xs lg:text-sm font-medium">
                              {funcionario.pontoHoje ? <div className="flex items-center gap-2">
                                  {funcionario.statusPonto === 'pending' && <>
                                      <button onClick={() => aprovarPonto(funcionario.id, funcionario.pontoHoje.id)} className="text-green-600 hover:text-green-900 inline-flex items-center gap-0.5 sm:gap-1 px-0.5 sm:px-1 lg:px-2 py-0.5 sm:py-0.5 lg:py-1 rounded hover:bg-green-50 transition-colors text-[9px] sm:text-[10px] md:text-xs" title="Aprovar ponto">
                                        <FiCheck className="w-3 h-3 sm:w-3 sm:h-3 lg:w-4 lg:h-4" /> <span className="hidden sm:inline">{t('admin.approve')}</span>
                                      </button>
                                      <button onClick={() => desaprovarPonto(funcionario.id, funcionario.pontoHoje.id)} className="text-red-600 hover:text-red-900 inline-flex items-center gap-0.5 sm:gap-1 px-0.5 sm:px-1 lg:px-2 py-0.5 sm:py-0.5 lg:py-1 rounded hover:bg-red-50 transition-colors text-[9px] sm:text-[10px] md:text-xs" title="Desaprovar ponto">
                                        <FiXCircle className="w-3 h-3 sm:w-3 sm:h-3 lg:w-4 lg:h-4" /> <span className="hidden sm:inline">{t('admin.disapprove')}</span>
                                      </button>
                                    </>}
                                  {funcionario.statusPonto === 'completed' && <button onClick={() => desaprovarPonto(funcionario.id, funcionario.pontoHoje.id)} className="text-red-600 hover:text-red-900 inline-flex items-center gap-0.5 sm:gap-1 px-0.5 sm:px-1 lg:px-2 py-0.5 sm:py-0.5 lg:py-1 rounded hover:bg-red-50 transition-colors text-[9px] sm:text-[10px] md:text-xs" title="Desaprovar ponto">
                                      <FiXCircle className="w-3 h-3 sm:w-3 sm:h-3 lg:w-4 lg:h-4" /> <span className="hidden sm:inline">{t('admin.disapprove')}</span>
                                    </button>}
                                  {funcionario.statusPonto === 'rejected' && <button onClick={() => aprovarPonto(funcionario.id, funcionario.pontoHoje.id)} className="text-green-600 hover:text-green-900 inline-flex items-center gap-0.5 sm:gap-1 px-0.5 sm:px-1 lg:px-2 py-0.5 sm:py-0.5 lg:py-1 rounded hover:bg-green-50 transition-colors text-[9px] sm:text-[10px] md:text-xs" title="Aprovar ponto">
                                      <FiCheck className="w-3 h-3 sm:w-3 sm:h-3 lg:w-4 lg:h-4" /> <span className="hidden sm:inline">{t('admin.approve')}</span>
                                    </button>}
                ...
                {loading ? <div className="text-center py-8">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {funcionarios.filter(f => f.is_active === false).length === 0 ? <tr>
                          <td colSpan="5" className="px-0.5 sm:px-2 md:px-4 lg:px-6 py-12 text-center text-gray-500 text-[9px] sm:text-[10px] md:text-xs lg:text-sm">
                        ...
                        </tr> : funcionarios.filter(f => f.is_active === false).map(funcionario => <tr key={funcionario.id} className="hover:bg-gray-50">
                            <td className="px-0 sm:px-2 md:px-3 lg:px-4 xl:px-6 py-2 sm:py-3 lg:py-4 whitespace-nowrap">
                              <div className="relative">
                                <button onClick={() => setMenuAberto(menuAberto === funcionario.id ? null : funcionario.id)} className="p-1 sm:p-1.5 lg:p-2 hover:bg-gray-100 rounded-full transition-colors" title="Opções">
                                  <FiMoreVertical className="w-3.5 h-3.5 sm:w-4 sm:h-4 lg:w-5 lg:h-5 text-gray-600" />
                                </button>
                            ...
                            <td className="px-0 sm:px-2 md:px-3 lg:px-4 xl:px-6 py-2 sm:py-3 lg:py-4 whitespace-nowrap">
                              <div>
                                {!funcionario.cargo && !funcionario.departamento ? <div className="text-[9px] sm:text-[10px] md:text-xs lg:text-sm font-medium text-gray-400">---</div> : <>
                                    <div className="text-[9px] sm:text-[10px] md:text-xs lg:text-sm font-medium text-gray-900">
                                      {funcionario.cargo || '---'}
                                    </div>
                                    {funcionario.departamento && <div className="text-[8px] sm:text-[9px] md:text-xs text-gray-500">{funcionario.departamento}</div>}
                                  </>}
                              </div>
                            </td>
                            <td className="px-0 sm:px-2 md:px-3 lg:px-4 xl:px-6 py-2 sm:py-3 lg:py-4 whitespace-nowrap text-[9px] sm:text-[10px] md:text-xs lg:text-sm text-gray-900">
