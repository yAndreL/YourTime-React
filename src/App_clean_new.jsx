      ...
      <div className="space-y-6">
        {error && <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-200 px-4 py-3 rounded-md mb-6">
          ...
          return `${horas.toString().padStart(2, '0')}:${minutos.toString().padStart(2, '0')}`;
              ...
              {selectedProject && selectedProject.horas_estimadas && <div className="yt-card p-6 mb-6">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('dashboard.hoursProgress')}</h3>
            ...
            {showProjectsSkeleton ? <ProjectsLoadingSkeleton count={4} /> : currentProjects.length > 0 ? <>
              ...
              maxWidth: '320px'
                ...
                {projectsWithHours.length > CARDS_PER_PAGE && <div className="flex items-center justify-center gap-4 mt-6">
                    <button type="button" onClick={handlePrevPage} disabled={currentPage === 0} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${currentPage === 0 ? 'bg-gray-100 dark:bg-gray-800 text-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-sm hover:shadow-md'}`}>
                ...
                setTimeout(() => {
                  ...
                  setFadeTransition(false);
                    ...
                    <button type="button" onClick={handleNextPage} disabled={currentPage === totalPages - 1} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${currentPage === totalPages - 1 ? 'bg-gray-100 dark:bg-gray-800 text-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-sm hover:shadow-md'}`}>
                      {t('dashboard.next')}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            <div className="lg:col-span-2 yt-card p-6">
              ...
              {error ? <div className="flex items-center justify-center h-32 text-red-500 dark:text-red-400">
                ...
                return weeklyData.map((item, index) => {
                  ...
                  if (totalHours === 0) {
                    return <div key={index} className="flex items-center gap-4">
                              <div className="w-20 flex-shrink-0">
                                <span className="text-sm text-gray-600 dark:text-gray-300">{item.dia}</span>
                              </div>
                              <div className="flex-1">
                                <div className="bg-gray-200 dark:bg-gray-700 rounded-full h-2"></div>
                              </div>
                              <div className="w-12 flex-shrink-0 text-right">
                                <span className="text-sm font-medium text-gray-900 dark:text-gray-100">0:00h</span>
                              </div>
                            </div>;
                  }
                  ...
                  return <div key={index} className="flex items-center gap-4">
                            <div className="w-20 flex-shrink-0">
                              <span className={`text-sm ${isPending ? 'text-yellow-600 dark:text-yellow-400 font-semibold' : item.isToday ? 'text-green-600 dark:text-green-400 font-semibold' : 'text-gray-600 dark:text-gray-300'}`}>
                          ...
                          backgroundImage: `repeating-linear-gradient(
                            ...
                            <div className="w-12 flex-shrink-0 text-right">
                              <span className={`text-sm font-medium ${isPending ? 'text-yellow-600 dark:text-yellow-400' : item.isToday ? 'text-green-600 dark:text-green-400' : 'text-gray-900 dark:text-gray-100'}`}>
                                {item.horas}h
                              </span>
                            </div>
                          </div>;
          ...
          <div className="yt-card p-6 mb-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">{t('dashboard.recentActivities')}</h3>

            {!loading && !error && timeRecords?.length > 0 ? <div className="space-y-3">
                {timeRecords.slice(-5).reverse().map((record, index) => <div key={record.id || index} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-800 last:border-b-0">
          ...
          <div className="yt-card p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">{t('dashboard.hoursThisWeek')}</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-green-600">
                  {userData?.carga_horaria ? `${userData.carga_horaria}h${t('dashboard.perWeek')}` : `40h${t('dashboard.perWeek')}`}
                </p>
                <p className="text-sm text-gray-500 dark:text-gray-400">{t('dashboard.workload')}</p>
              </div>
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">{t('dashboard.weekGoal')}</p>
              </div>
            </div>
          </div>
