...
function TimeRecordsSummary({
  ...
  if (error) {
    return <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {onRefresh && <button onClick={onRefresh} className="text-sm text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-1">
  ...
  if (!timeRecords || timeRecords.length === 0) {
    return <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <div className="text-center py-8">
          ...
          <button onClick={() => window.location.href = '/formulario-ponto'} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors">
  ...
  return <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">{t('dashboard.timeRecordSummary')}</h3>
        <button onClick={() => setShowDetails(!showDetails)} className="flex items-center gap-1 text-sm text-blue-600 hover:text-blue-800 transition-colors">
          {showDetails ? <>
      ...
      {showDetails && <div className="border-t pt-4">
          <h4 className="text-md font-semibold text-gray-900 mb-3">{t('dashboard.detailedRecords')}</h4>
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {timeRecords.map((record, index) => <div key={record.id || index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex-1">
                  ...
                  {record.observacao && <div className="text-sm text-blue-600 mt-1 flex items-center gap-1">
