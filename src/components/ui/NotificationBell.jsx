...
function NotificationBell() {
  ...
  return <div className="relative" ref={dropdownRef}>
      <button onClick={handleToggleDropdown} className="relative p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors" aria-label="Notificações">
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
        </svg>
        
        {unreadCount > 0 && <span className="absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white transform translate-x-1/2 -translate-y-1/2 bg-red-500 rounded-full">
            {unreadCount > 99 ? '99+' : unreadCount}
          </span>}
      </button>

      {isOpen && <div className="absolute right-0 mt-2 w-96 max-[620px]:w-[min(calc(100vw-1rem),250px)] max-[620px]:right-0 max-[620px]:mr-0.5 bg-white rounded-lg shadow-xl border border-gray-200 z-50 max-h-[600px] flex flex-col">
          <div className="px-3 max-[620px]:px-1.5 py-3 max-[620px]:py-2 border-b border-gray-200 flex items-center justify-between bg-gray-50 rounded-t-lg gap-2">
            ...
            {unreadCount > 0 && <button onClick={handleMarcarTodasComoLidas} className="text-[10px] max-[620px]:text-[8px] text-blue-600 hover:text-blue-800 font-medium whitespace-nowrap flex-shrink-0">
          ...
          <div className="overflow-y-auto flex-1">
            {isLoading ? <div className="p-8 text-center text-gray-500">
              ...
              </div> : notifications.length === 0 ? <div className="p-8 max-[620px]:p-4 text-center text-gray-500">
              ...
              </div> : notifications.map(notificacao => <div key={notificacao.id} onClick={() => handleNotificacaoClick(notificacao)} className={`px-4 max-[620px]:px-2 py-3 max-[620px]:py-2 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors ${!notificacao.lida ? 'bg-blue-50' : ''}`}>
                  <div className="flex items-start gap-3 max-[620px]:gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {!notificacao.lida && <div className="w-2 h-2 bg-blue-600 rounded-full flex-shrink-0 mt-1"></div>}
                      </div>
                      ...
                      {notificacao.metadata?.data_formatada && <p className="text-xs max-[620px]:text-[10px] text-gray-500 mt-1">{t('notifications.date')} {notificacao.metadata.data_formatada}</p>}
                      <p className="text-xs max-[620px]:text-[10px] text-gray-400 mt-1">{formatarTempo(notificacao.created_at)}</p>
                    </div>

                    <div className="flex flex-col gap-1 flex-shrink-0">
                      {!notificacao.lida && <button onClick={e => handleMarcarComoLida(notificacao.id, e)} className="text-blue-600 hover:text-blue-800 p-1" title="Marcar como lida">
                          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                          </svg>
                        </button>}
                      <button onClick={e => handleDeletarNotificacao(notificacao.id, e)} className="text-red-600 hover:text-red-800 p-1" title="Deletar">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          ...
          {notifications.length > 0 && <div className="px-4 py-3 border-t border-gray-200 bg-gray-50 rounded-b-lg">
              <button onClick={() => {
