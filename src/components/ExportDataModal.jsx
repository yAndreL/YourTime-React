  ...
  const gerarCSVSimples = async () => {
    ...
    try {
      ...
      for (const funcionarioId of funcionariosSelecionados) {
        ...
        csvLines.push('');
        csvLines.push([t('export.date'), t('export.entry1'), t('export.exit1'), t('export.entry2'), t('export.exit2'), t('export.totalHours'), t('export.status'), t('notifications.project')].join(','));
        registros.forEach(reg => {
          ...
          csvLines.push([formatarData(reg.data), reg.entrada1 || '', reg.saida1 || '', reg.entrada2 || '', reg.saida2 || '', formatarHoras(totalDia), traduzirStatus(reg.status), escaparCSV(reg.projetos?.nome || '')].join(','));
        });
        csvLines.push('');
        csvLines.push('---');
        csvLines.push('');
      ...
      const nomeArquivo = funcionariosSelecionados.length === 1 ? `${t('export.fileName')}_${funcionarios.find(f => f.id === funcionariosSelecionados[0])?.nome?.replace(/\s+/g, '_')}_${formatarData(dataInicio)}_a_${formatarData(dataFim)}.csv` : `${t('export.fileName')}_multiple_${formatarData(dataInicio)}_a_${formatarData(dataFim)}.csv`;
      link.href = url;
  ...
  const gerarXLSX = async () => {
    ...
    try {
      ...
      for (const funcionarioId of funcionariosSelecionados) {
        ...
        row++;
        const dadosFuncionario = [[t('profile.name'), funcionario.nome], [t('profile.email'), funcionario.email], [t('profile.position'), funcionario.cargo || t('export.notDefined')], [t('profile.department'), funcionario.departamento || t('export.notDefined')]];
        ...
        row++;
        const startRow = row;
        const stats = [[t('export.totalHours'), formatarHoras(totalHorasTrabalhadas)], [t('export.overtimeHours'), formatarHoras(totalHorasExtras)], [t('export.workedDays'), diasUteis.toString()], [t('export.averagePerDay'), formatarHoras(totalHorasTrabalhadas / diasUteis)]];
        ...
        row = startRow;
        const statusDist = [[t('export.approved'), diasAprovados, (diasAprovados / diasUteis * 100).toFixed(1) + '%', 'FF dcfce7', 'FF166534'], [t('export.pending'), diasPendentes, (diasPendentes / diasUteis * 100).toFixed(1) + '%', 'FFfef3c7', 'FF92400e'], [t('export.rejected'), diasRejeitados, (diasRejeitados / diasUteis * 100).toFixed(1) + '%', 'FFfee2e2', 'FF991b1b']].filter(([, qtd]) => qtd > 0);
        ...
        horasPorDia.forEach(dia => {
          ...
          const statusColor = dia.status === t('export.approved') ? ['FFdcfce7', 'FF166534'] : dia.status === t('export.pending') ? ['FFfef3c7', 'FF92400e'] : ['FFfee2e2', 'FF991b1b'];
          statusCell.style = {
            ...cellStyle,
            alignment: {
              horizontal: 'center'
        ...
        worksheet.getCell(`B${row}`).style = sectionStyle;
        row++;
        const detailHeaders = [t('export.date'), t('export.entry1'), t('export.exit1'), t('export.breakStart'), t('export.breakEnd'), t('export.duration'), t('export.entry2'), t('export.exit2'), t('export.total'), t('export.status'), t('notifications.project')];
        ...
        registrosDetalhados.forEach(reg => {
          ...
          const statusColor = reg.status === t('export.approved') ? ['FFdcfce7', 'FF166534'] : reg.status === t('export.pending') ? ['FFfef3c7', 'FF92400e'] : ['FFfee2e2', 'FF991b1b'];
          statusCell.style = {
            ...cellStyle,
            alignment: {
              horizontal: 'center'
      ...
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      const nomeArquivo = funcionariosSelecionados.length === 1 ? `${t('export.fileName')}_${funcionarios.find(f => f.id === funcionariosSelecionados[0])?.nome?.replace(/\s+/g, '_')}_${formatarData(dataInicio)}_a_${formatarData(dataFim)}.xlsx` : `${t('export.fileName')}_multiple_${formatarData(dataInicio)}_a_${formatarData(dataFim)}.xlsx`;
      link.href = url;
      link.download = nomeArquivo;
      document.body.appendChild(link);
      link.click();
